...
def test_signal_to_concentration_S_to_ep_SPGR():
    ...
    # Ground truth is output at the time of writing
    S = np.array(
        [7, 9, 6, 10, 9, 6, 9, 10, 9, 9, 9, 12, 8, 10, 12, 15, 53, 70, 71, 70, 63, 58, 54, 50, 53, 48, 52, 49, 42],
        dtype=np.float64)
    S_BL = S[0]  # use first point for baseline signal
    ...
    R_1_truth = np.array([0.71428571, 0.93303587, 0.6074698, 1.04505254, 0.93303587, 0.6074698,
    ...
    np.testing.assert_allclose(R_1_truth, R_1, rtol=0, atol=1e-7)
    return


def test_signal_to_concentration_ep_to_C_R1_lin_rxy():
    # 1. Simple use case
    # Test data and ground truth generated by test writer
